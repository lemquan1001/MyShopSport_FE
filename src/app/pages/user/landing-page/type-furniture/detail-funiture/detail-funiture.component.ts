...
export class DetailFunitureComponent implements OnInit {
  ...
  buttons = [
    ...
    { id: 6, color: '', colorText: '', size: '3XL' },
  ];
  selectedButtonId: number | null = null;
  productDetailSize: any;
  isActiveBtnAddToCart: boolean = false;
  constructor(public detailProduct: DetailProductServiceService) {}
  ...
  addToCard(product: ProductDetail, size: string) {
    this.detailProduct
      .getDetailProductBySize(product.product.id, size)
      .subscribe((res) => {
  ...
  confirmAddToCard(product: ProductDetail) {
    if (this.detailProduct.carts.value.length === 0) {
      ...
      for (let i = 0; i < this.detailProduct.carts.value.length; i++) {
        if (
          this.productDetailSize.id === this.detailProduct.carts.value[i].id &&
          this.productDetailSize.product.id ===
            this.detailProduct.carts.value[i].product.id
        ) {
  ...
  selectSize(size: string, buttonId: number) {
    ...
    if (this.selectedButtonId !== null) {
      const previousButton = this.buttons.find(
        (button) => button.id === this.selectedButtonId
      );
      if (previousButton) {
